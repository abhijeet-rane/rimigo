...
const SheetOverlay = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Overlay>, React.ComponentPropsWithoutRef<typeof SheetPrimitive.Overlay>>(
    ({ className, ...props }, ref) => (
        <SheetPrimitive.Overlay
            className={cn(
                'fixed inset-0 z-50 bg-black/80 transition-opacity duration-2000 ease-out data-[state=closed]:opacity-0 data-[state=open]:opacity-100',
                className
            )}
            {...props}
            ref={ref}
...
const sheetVariants = cva('fixed z-50 gap-4 bg-background p-6 shadow-lg', {
    variants: {
        side: {
            ...
            left: 'inset-y-0 left-0 h-full w-1/2 border-r',
            right: 'inset-y-0 right-0 h-full w-1/2 border-l'
        }
    },
    defaultVariants: {
        side: 'right'
