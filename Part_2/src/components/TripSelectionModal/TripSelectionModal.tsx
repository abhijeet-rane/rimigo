...
interface TripSelectionModalProps {
    ...
    /** When true, renders content inline without the modal overlay/backdrop */
    inline?: boolean
}
...
const TripSelectionModal: React.FC<TripSelectionModalProps> = ({
    ...
    const handleCreateNewTrip = async () => {
        ...
        try {
            ...
            if (wizardState?.startDate && wizardState?.endDate) {
                const start = wizardState.startDate instanceof Date ? wizardState.startDate : new Date(wizardState.startDate)
                const end = wizardState.endDate instanceof Date ? wizardState.endDate : new Date(wizardState.endDate)
    ...
    const cardContent = (
        <div className={inline ? 'flex flex-col w-full' : 'bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col overflow-hidden'}>
            {/* Header */}
            <div className={`flex items-center justify-between ${inline ? 'pb-4' : 'p-5 border-b border-grey-4/50'}`}>
                <div>
                    <h2 className="text-lg font-bold font-red-hat-display text-grey-0">Welcome back!</h2>
                    <p className="text-sm text-grey-2 font-manrope font-[500] mt-0.5">Continue with an existing trip or create a new one</p>
                </div>
                {!inline && !isCreating && !isSelectingTrip && (
                    <button
                        type="button"
                        onClick={onClose}
            ...
            {/* New trip action */}
            <div className={inline ? 'pb-2' : 'px-5 pt-4 pb-2'}>
                <p className="text-xs font-bold font-red-hat-display text-grey-2 uppercase tracking-wide pb-[8px]">New trip</p>
                <button
                    type="button"
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-sm font-semibold text-primary-default font-red-hat-display truncate">
                            {isCreating
                                ? 'Creating your trip...'
                                : pendingWizardData?.destinations?.length === 0
                                    ? 'Unplanned trip'
                    ...
                    {isCreating ? (
                        <Loader2
                            size={16}
                            className="text-primary-default animate-spin shrink-0"
                        />
                    ) : (
            ...
            {/* Trip list */}
            <div className={`flex-1 overflow-y-auto ${inline ? 'py-3' : 'px-5 py-3'}`}>
                {/* Existing trips */}
                {isTripsLoading ? (
                    <div className="flex items-center justify-center py-6">
                        <Loader2
                            size={20}
                            className="text-grey-3 animate-spin"
                        />
                    </div>
                ) : tripsWithDestinations.length > 0 ? (
                    <div className="flex flex-col gap-2">
                        <p className="text-xs font-bold font-red-hat-display text-grey-2 uppercase tracking-wide">Your existing trips</p>
                        {ownTrips.map((trip) => (
                            <div
                                key={trip.trip_id}
                                className="relative">
