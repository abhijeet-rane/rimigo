...
const OutputLoadingComponent: React.FC<OutputLoadingComponentProps> = ({ status, className = '', uiConfig, elapsedMs = 0 }) => {
    ...
    useEffect(() => {
        if (status === 'in_progress' || status === 'queued') {
            ...
            let startStep = 0
            const remToStep1 = Math.max(0, d0 - elapsedMs)
            const remToStep2 = Math.max(0, d0 + d1 - elapsedMs)
    ...
    return (
        <div className={`bg-grey_6 rounded-2xl p-6 ${className}`}>
            <div className="flex">
                {/* Timeline */}
                <div className="flex flex-col items-center mr-6">
                    {steps.map((step, index) => {
                        ...
                        let connectorHeightPx = 68 // default ~ h-16
                        if (isActive && step.id === 'scanning') {
                            connectorHeightPx = 190 // with sub-components
                            ...
                            <div
                                ...
                                {isActive && step.id === 'scanning' && (
                                    <div className="space-y-4">
                                        ...
                                        {cfg && cfg.scanning && cfg.scanning.providers && cfg.scanning.providers.length > 0 && (
                                            <div className="flex items-center gap-2 bg-white rounded-[8px] px-4 py-3 w-fit">
                                ...
                                {isActive && step.id === 'analyzing' && (
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div
                                                className="w-6 h-6 border border-primary-default rounded-full animate-pulse"
